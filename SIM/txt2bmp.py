"""
python txt2bmp.py <txt_input_folder> <origin_img_folder> <output_folder>

将 txt 转为 bmp，并且将原始的 png 复制过来对比
"""
...
def process_txt_file(txt_path, origin_img_folder, output_folder):
    ...
    # 将图像保存为BMP文件，支持透明通道
    # img.save(output_path, format='BMP', transparency=0)
    img.save(output_path, format='BMP')

    print("BMP图片已创建:", output_path)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("usage: python txt2bmp.py <txt_input_folder> <origin_img_folder> <output_folder>")
        sys.exit(1)
